  ...
  <view class="barn-list-container">
    ...
    <view class="base-selector">
      ...
      <view class="selector-value" @click="showBaseSelector">
        <text>{{ currentBase?.name || '请选择基地' }}</text>
      ...
      <view v-else-if="filteredBarns.length > 0">
        ...
        >
          <view class="barn-header">
            ...
            <view class="barn-status" :class="getBarnStatusClass(barn)">
...
const { bases, currentBase } = baseStore
const barns = ref([])
...
const loadBarns = async () => {
  ...
  try {
    const response = await uni.request({
      url: `/api/v1/barns?baseId=${currentBase.value.id}`,
      method: 'GET'
    })
    ...
    if (response.data.success) {
      ...
      // 缓存牛棚数据
      cacheStore.setCacheData(`barns_${currentBase.value.id}`, barns.value)
    } else {
    ...
    // 尝试从缓存加载
    const cachedBarns = cacheStore.getCacheData(`barns_${currentBase.value.id}`)
    if (cachedBarns) {
