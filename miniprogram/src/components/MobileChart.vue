      ...
      <view class="bar-container">
        ...
        >
          <view class="bar-column">
            <view 
              class="bar-fill"
              :style="{ backgroundColor: item.color || colors[index % colors.length], height: Math.round((item.value / Math.max(...chartData.map(d => d.value))) * 100) + '%' }"
            ></view>
          </view>
...
export default {
  ...
  methods: {
    initTrendChart() {
      this.ctx = uni.createCanvasContext('trendCanvas')
      this.drawTrendChart()
    },
    ...
    drawTrendChart() {
      ...
      for (let i = 0; i <= 4; i++) {
        ...
        this.ctx.lineTo(canvasWidth - padding, y)
        this.ctx.stroke()
      }
      
      // 绘制数据线
      this.chartData.forEach((series, seriesIndex) => {
