  ...
  methods: {
    async loadDashboardData(silent = false) {
      ...
      try {
        ...
        await dashboardStore.refreshAll(this.currentBase.id)
        this.keyIndicators = dashboardStore.keyIndicators
        this.pendingTasks = dashboardStore.pendingTasks
        this.loading = dashboardStore.loading
        this.lastUpdated = dashboardStore.lastUpdated
    ...
    navigateTo(url) {
      ...
      const fullUrl = `${url}${separator}baseId=${this.currentBase.id}`
    ...
    showRevenueDetail() {
      uni.showModal({
        title: '月度收入详情',
        content: `当前月度收入: ${this.formatCurrency(this.keyIndicators.monthlyRevenue || 0)}`,
        showCancel: false
      })
    },
