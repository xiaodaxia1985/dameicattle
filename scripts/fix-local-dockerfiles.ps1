...
Write-Host "=== Fixing Local Dockerfiles ===" -ForegroundColor Green
...
# 首先复制Node.js包到每个服务目录
Write-Host "Copying Node.js package to each service directory..." -ForegroundColor Yellow
foreach ($service in $services) {
    if (Test-Path $service) {
        Copy-Item "node-v22.17.0-linux-x64-musl.tar.xz" "$service/" -Force
        Write-Host "  ✓ Copied to $service/" -ForegroundColor Green
    }
}
...
Write-Host "Updating Dockerfiles..." -ForegroundColor Yellow
foreach ($service in $services) {
    if (Test-Path $service) {
        Write-Host "Updating Dockerfile.local for $service..." -ForegroundColor Cyan
        ...
        Write-Host "  ✓ Updated $service/Dockerfile.local" -ForegroundColor Green
    }
}

Set-Location $originalPath

Write-Host "`n=== Dockerfiles fixed! ===" -ForegroundColor Green
Write-Host "Node.js package copied to each service and Dockerfiles updated." -ForegroundColor Cyan